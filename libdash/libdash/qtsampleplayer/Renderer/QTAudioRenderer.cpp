...
void                    QTAudioRenderer::Init           ()
{
    ...
    //std::cout << "Default device info is NULL? " << deviceInfo.isNull() << std::endl;
    ...
    //std::cout << "Supported? " << this->deviceInfo.isFormatSupported(this->format) << std::endl;
    for (int i = 0; i < deviceInfo.supportedCodecs().size(); ++i)
        std::cout << deviceInfo.supportedCodecs().at(i).toLocal8Bit().constData() << std::endl;

    if (!this->deviceInfo.isFormatSupported(this->format))
        format = this->deviceInfo.nearestFormat(this->format);
    //std::cout<<"onTimeout of Interest: "<< format << std::endl;
...
void                    QTAudioRenderer::Reset          ()
{
    ...
    //std::cout << "audioOutput: " << this->audioOutput << std::endl;
