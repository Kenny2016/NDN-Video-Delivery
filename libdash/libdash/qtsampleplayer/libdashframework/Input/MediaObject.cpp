...
#include "../../../libdash/include/IDownloadObserver.h"
...
MediaObject::MediaObject    (ISegment *segment, IRepresentation *rep) :
             ...
             rep            (rep)
{
    std::cout<<"create MediaObject"<<std::endl;
...
{
    std::cout<<"start create mediaobject::startdownload non parameter conn"<<std::endl;
    NDNConnection* conn = new NDNConnection();
...
bool                MediaObject::StartDownload          (NDNConnection* conn)
{
    std::cout<<"start create mediaobject::startdownload parameterable conn"<<std::endl;
    if(!conn->Init(this->segment))
            std::cout <<"INIT ERROR\n";
    ...
    std::cout<<"schedule finished!!!!"<<std::endl;
