...
/*    if(datrec->IsAudio())
    {
    	FILE * fp;
    	fp = fopen("/media/hdd/IORead.bin", "ab");
...
void                LibavDecoder::Notify                  (AVFrame *frame, StreamConfig *config)
{
    switch(config->stream->codec->codec_type)
    {
        ...
        {
        	/*     	FILE *myFile;
        	myFile = fopen("/media/hdd/decoded0.bin","ab");
        	fwrite(frame->data[0], sizeof(uint8_t), (uint32_t)(frame->linesize[0]),myFile);
        	fclose(myFile);
        	myFile = fopen("/media/hdd/decoded1.bin","ab");
        	fwrite(frame->data[1], sizeof(uint8_t), (uint32_t)(frame->linesize[0]),myFile);
        	fclose(myFile);
   */
