...
{
	MediaObject *mediaObject = this->mediaobjects.back();
...
{
	L("Media_object_buffer: %s\t%f\n", (this->type == sampleplayer::managers::StreamType::AUDIO)? "Audio": "Video" ,((double) this->mediaobjects.size()/(double) this->maxcapacity));
    for(size_t i = 0; i < this->observer.size(); i++)
