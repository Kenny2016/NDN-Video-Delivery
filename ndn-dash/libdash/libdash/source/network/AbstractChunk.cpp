...
bool    AbstractChunk::StartDownload                ()
{
    if(this->stateManager.State() != NOT_STARTED)
        return false;
    L("TCP_CONNECTION:\tSTARTING DOWNLOAD %s\n", this->AbsoluteURI().c_str());
...
void*   AbstractChunk::DownloadInternalConnection   (void *abstractchunk)
{
    ...
    speed = 8*speed;
    size = 8*size;
    L("TCP_CONNECTION:\tFINISHED DOWNLOAD %s, Average_DL: %f, size_DL: %d\n", chunk->AbsoluteURI().c_str(), speed, (int)size);
    chunk->NotifyDownloadRateChanged(speed);
